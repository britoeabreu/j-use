...
public class ProgramingFacade extends BasicFacade implements JUSE_ProgramingFacade
{
	...
	public String statistics(boolean verbose)
	{
		...
		{
			result+=
				getSystem().model().classes().size() + "\t" +
				getSystem().model().associations().size() + "\t" +
				getSystem().model().classInvariants().size() + "\t" + 
				getSystem().model().prePostConditions().size() + "\t" + 
	...
	/* (non-Javadoc)
	 * @see org.quasar.juse.api.JUSE_ProgramingFacade#setLinkObjectAttribute(org.tzi.use.uml.sys.MLinkObject, org.tzi.use.uml.mm.MAttribute, org.tzi.use.uml.ocl.value.Value)
	 */
	...
	/* (non-Javadoc)
	 * @see org.quasar.juse.api.JUSE_ProgramingFacade#getLinkObjectAttribute(org.tzi.use.uml.sys.MLinkObject, org.tzi.use.uml.mm.MAttribute)
	 */
	...
	{
		return this.deleteObject(theLinkObject);
	}
	 ...
	 */
	public DeleteObjectResult deleteLinkObject(MAssociationClass theAssociationClass, List<MObject> members)
	{
		return this.deleteLink(theAssociationClass, members);
	}
	 ...
	 */
	public DeleteObjectResult deleteLink(MLink theLink)
	{
		assert theLink != null;
		...
		try
		{
			result = getSystem().state().deleteLink(theLink);
		}
		catch (MSystemException e)
		{
			e.printStackTrace();
